...
import { fetchAccountFills, fetchOpenBalances, fetchRedemptions, type OpenBalanceRow, type RedemptionRow } from "./indexer";
...
export async function loadHistory(ctx: LucidContext, account: `0x${string}`): Promise<HistoryEntry[]> {
  ...
  for (const r of redemptions) {
    try {
      let cb = fillsByMarket.get(r.marketId);
      if (!cb) {
        const fills = await fetchAccountFills(ctx.config.indexerUrl, r.marketId, account);
        cb = computeCostBasis(fills, account, decimals);
        fillsByMarket.set(r.marketId, cb);
      }
